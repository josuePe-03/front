  ...
  reducers: {
    ...
    onDeleteOperador: (state) => {
      if (state.activeOperador) {
        state.operadores = state.operadores.filter(
          (event) => event.id !== state.activeOperador.id
        );
        state.activeOperador = null;
      }
    },
    ...
    // FILTROS OPERADORES
    onLoadFiltrosOperador: (state, { payload = [] }) => {
      state.isLoadingOperadores = false;
      state.filtros = payload;
    },
    onLogoutModalOperador: (state) => {
      (state.isLoadingOperadores = false), (state.operador = []);
    },

    onLogoutOperador: (state) => {
      (state.isLoadingOperadores = true), (state.operadores = []);
      stOperador = null;
    },
  },
});
